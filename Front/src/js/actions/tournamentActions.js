...
export function getSingleEliminationMatches(tournamentId) {
  return function(dispatch) {
    dispatch({
      type: 'FETCH_SINGLE_ELIMINATION_MATCHES',
      payload: ''
    });
...
export function getDoubleEliminationMatches(tournamentId) {
  return function(dispatch) {
    dispatch({
      type: 'FETCH_DOUBLE_ELIMINATION_MATCHES',
      payload: ''
    });
    axios.get(serverDetails.baseUrl + '/tournaments/' + tournamentId + '/double-elimination/')
      .then((result) => {
        console.log(result);
        //var matches = helper.convertJSONtoMatches(result);
      })
      .catch((error) => {
        dispatch({
          type: 'FETCH_DOUBLE_ELIMINATION_MATCHES_REJECTED',
          payload: error
        })
      })
  }
...
export function startDoubleElimination(tournament, participants) {
  return function(dispatch) {
    while (participants.length < tournament.size) {
      participants.push(new Player(99, 'Walk', 'Over'));
    }
    shuffle(participants);
    var matches = helper.createDoubleEliminationMatches(tournament.size, participants);
    console.log(matches);
    axios.post(serverDetails.baseUrl + '/tournaments/' + tournament.id + '/double-elimination/', {
        matches: matches
      })
      .then((result) => {
        dispatch(view.showHomeScreen());
      ...
      .catch((error) => {
        dispatch({
          type: 'CREATE_DOUBLE_ELIMINATION_MATCHES_FAILED',
          payload: error
        })
      })
  }
