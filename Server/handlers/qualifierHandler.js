  ...
  updateBracket(dbClient, leagueId, match) {
    ...
    if(match.key.match(/^[1-8]$/g)) {
      if(Number(match.key) % 2 === 0) {
        promises.push(this.updateQualifiersMatch(dbClient, leagueId, match.loserNextMatchKey, null, loser));
        promises.push(this.updateQualifiersMatch(dbClient, leagueId, match.winnerNextMatchKey, null, winner));
      }
      else {
        promises.push(this.updateQualifiersMatch(dbClient, leagueId, match.loserNextMatchKey, loser, null));
        promises.push(this.updateQualifiersMatch(dbClient, leagueId, match.winnerNextMatchKey, winner, null));
      }
    }
    //WINNERS BRACKET
    if(match.key.match(/^[A|B].{1,2}$/g)) {
      var matchNum = Number(match.key.substring(1));
      if(matchNum % 2 === 0) {
        promises.push(this.updateQualifiersMatch(dbClient, leagueId, match.winnerNextMatchKey, null, winner));
      }
      else {
    ...
    if(match.key.match(/^L.{1,2}$/g)){
      var num = Number(match.key.split('L')[1]);
